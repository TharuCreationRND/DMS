...
from django.contrib import messages
from .models import ClassChange
from .filters import ClassChangeFilter
import pandas as pd
...
@login_required
def upload_classchange_excel(request):
    if request.method == 'POST' and request.FILES.get('file'):
        ...
        try:
            ...
            # Loop through rows and create ClassChange instances
            for _, row in df.iterrows():
                # Adjust column names exactly to your Excel sheet headers
                ClassChange.objects.create(
                    date=row['Date'],
                    time=row.get('Time') if 'Time' in row else None,
                    center=row['Center'],
                    vin=row['Vehicle Identification Number'],
                    previous_class=row['Prevous Class'],
                    change_class=row['New Class'],
                    reason=row['Reason'],
                    approved_by=row['Approved By'],
                    refund=True if str(row['Refund']).strip().lower() in ['yes', 'true', '1'] else False,
                    remark=row['Remark'],
                    updated_by=request.user,
                )
            messages.success(request, "Class Change Excel data imported successfully!")
            return redirect('class_change_list')
        except Exception as e:
            messages.error(request, f"Failed to import Excel file: {str(e)}")

    # If GET or no file, just redirect or you can render a form page
...
def class_change_list(request):
    if request.method == 'POST' and request.FILES.get('file'):
        ...
        try:
            ...
            for _, row in df.iterrows():
                # Check if record already exists (optional, to avoid duplicates)
                obj, created = ClassChange.objects.get_or_create(
                    date=row['Date'],
                    time=row['Time'] if pd.notna(row['Time']) else None,
                    center=row['Center'],
                    vin=row['VIN'],
                    previous_class=row['Previous Class'],
